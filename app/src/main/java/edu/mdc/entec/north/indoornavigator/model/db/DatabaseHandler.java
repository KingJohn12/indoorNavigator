package edu.mdc.entec.north.indoornavigator.model.db;
...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    //TODO
    private static String DB_DIR = "/data/data/edu.mdc.entec.north.indoornavigator/databases/";
...
*/
    // Getting a cell
    CellCoordinates getCell(int X, int Y) {
        ...
        try {
            cell = new CellCoordinates(Integer.parseInt(cursor.getString(0)),
    ...
    //Get the room from the cell coordinates
    public Room getRoomAtCoordinates(CellCoordinates cellCoordinates) {
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = db.query("Cell",                    // table name
                new String[]{"X", "Y", "RoomID"},   // list of which table columns to return
                " X=? AND Y=? ",  // WHERE clause, with ?  as placeholders for parameters
                new String[]{String.valueOf(cellCoordinates.getX()), String.valueOf(cellCoordinates.getY())}, // parameters for the placeholders in the WHERE clause
                null,                               // GROUP BY
                null,                               // HAVING
                null,                               // ORDER BY
                null);                              // ?
        ...
        Room room = null;
        try {
            room = getRoom(Integer.parseInt(cursor.getString(2)));
        } catch (CursorIndexOutOfBoundsException e) {
